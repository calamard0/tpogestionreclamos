...
import ar.edu.uade.model.EventoReclamo;
...
public class ReclamoDAO extends BaseDAO 
{
	...
	public Reclamo buscarReclamo(long numero)
	{
		...
		try
		{
			...
			while (result.next())
			{
				...
				r.setOperador(UsuarioDAO.getInstancia().buscarUsuario(usuario));
				r.setResponsable(UsuarioDAO.getInstancia().buscarUsuario(usuario));
